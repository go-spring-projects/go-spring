...
import (
	...
	pkg1 "github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/bar"
	pkg2 "github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/foo"
)

type SamePkg struct{}

func (p *SamePkg) Package() {
	fmt.Println("github.com/go-spring-projects/go-spring/internal/utils/utils.SamePkg")
}
...
func TestPkgPath(t *testing.T) {
	...
	data := []struct {
		...
		},
		{
			...
			reflect.Struct,
			"SamePkg",
			"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/bar",
		},
		{
		...
		},
		{
			...
			reflect.Struct,
			"SamePkg",
			"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/foo",
		},
		{
...
func TestTypeName(t *testing.T) {
	...
	data := map[interface{}]struct {
		...
		pkg1.SamePkg{}:                             {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/bar/pkg.SamePkg", "pkg.SamePkg"},
		new(pkg1.SamePkg):                          {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/bar/pkg.SamePkg", "*pkg.SamePkg"},
		reflect.TypeOf(make([]pkg1.SamePkg, 0)):    {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/bar/pkg.SamePkg", "[]pkg.SamePkg"},
		reflect.TypeOf(&[]pkg1.SamePkg{}):          {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/bar/pkg.SamePkg", "*[]pkg.SamePkg"},
		reflect.TypeOf(make(map[int]pkg1.SamePkg)): {"map[int]pkg.SamePkg", "map[int]pkg.SamePkg"},

		pkg2.SamePkg{}:                             {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/foo/pkg.SamePkg", "pkg.SamePkg"},
		new(pkg2.SamePkg):                          {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/foo/pkg.SamePkg", "*pkg.SamePkg"},
		reflect.TypeOf(make([]pkg2.SamePkg, 0)):    {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/foo/pkg.SamePkg", "[]pkg.SamePkg"},
		reflect.TypeOf(&[]pkg2.SamePkg{}):          {"github.com/go-spring-projects/go-spring/internal/utils/testdata/pkg/foo/pkg.SamePkg", "*[]pkg.SamePkg"},
		reflect.TypeOf(make(map[int]pkg2.SamePkg)): {"map[int]pkg.SamePkg", "map[int]pkg.SamePkg"},

		SamePkg{}:                             {"github.com/go-spring-projects/go-spring/internal/utils/utils.SamePkg", "utils.SamePkg"},
		new(SamePkg):                          {"github.com/go-spring-projects/go-spring/internal/utils/utils.SamePkg", "*utils.SamePkg"},
		reflect.TypeOf(make([]SamePkg, 0)):    {"github.com/go-spring-projects/go-spring/internal/utils/utils.SamePkg", "[]utils.SamePkg"},
		reflect.TypeOf(&[]SamePkg{}):          {"github.com/go-spring-projects/go-spring/internal/utils/utils.SamePkg", "*[]utils.SamePkg"},
		reflect.TypeOf(make(map[int]SamePkg)): {"map[int]utils.SamePkg", "map[int]utils.SamePkg"},
	}
...
func TestIsConverter(t *testing.T) {
	assert.False(t, IsConverter(reflect.TypeOf(3)))
	assert.False(t, IsConverter(reflect.TypeOf(func() {})))
	assert.False(t, IsConverter(reflect.TypeOf(func(key string) {})))
	assert.False(t, IsConverter(reflect.TypeOf(func(key string) string { return "" })))
	assert.True(t, IsConverter(reflect.TypeOf(func(key string) (string, error) { return "", nil })))
}
...
func TestReturnNothing(t *testing.T) {
	assert.True(t, ReturnNothing(reflect.TypeOf(func() {})))
	assert.True(t, ReturnNothing(reflect.TypeOf(func(key string) {})))
	assert.False(t, ReturnNothing(reflect.TypeOf(func() string { return "" })))
}

func TestReturnOnlyError(t *testing.T) {
	assert.True(t, ReturnOnlyError(reflect.TypeOf(func() error { return nil })))
	assert.True(t, ReturnOnlyError(reflect.TypeOf(func(string) error { return nil })))
	assert.False(t, ReturnOnlyError(reflect.TypeOf(func() (string, error) { return "", nil })))
}

func TestIsStructPtr(t *testing.T) {
	assert.False(t, IsStructPtr(reflect.TypeOf(3)))
	assert.False(t, IsStructPtr(reflect.TypeOf(func() {})))
	assert.False(t, IsStructPtr(reflect.TypeOf(struct{}{})))
	assert.False(t, IsStructPtr(reflect.TypeOf(struct{ a string }{})))
	assert.True(t, IsStructPtr(reflect.TypeOf(&struct{ a string }{})))
}

func TestIsConstructor(t *testing.T) {
	assert.False(t, IsConstructor(reflect.TypeOf(func() {})))
	assert.True(t, IsConstructor(reflect.TypeOf(func() string { return "" })))
	assert.True(t, IsConstructor(reflect.TypeOf(func() *string { return nil })))
	assert.True(t, IsConstructor(reflect.TypeOf(func() *testdata.Receiver { return nil })))
	assert.True(t, IsConstructor(reflect.TypeOf(func() (*testdata.Receiver, error) { return nil, nil })))
	assert.False(t, IsConstructor(reflect.TypeOf(func() (bool, *testdata.Receiver, error) { return false, nil, nil })))
}

func TestHasReceiver(t *testing.T) {
	assert.False(t, HasReceiver(reflect.TypeOf(func() {}), reflect.ValueOf(new(testdata.Receiver))))
	assert.True(t, HasReceiver(reflect.TypeOf(func(*testdata.Receiver) {}), reflect.ValueOf(new(testdata.Receiver))))
	assert.True(t, HasReceiver(reflect.TypeOf(func(*testdata.Receiver, int) {}), reflect.ValueOf(new(testdata.Receiver))))
	assert.True(t, HasReceiver(reflect.TypeOf(func(fmt.Stringer, int) {}), reflect.ValueOf(new(testdata.Receiver))))
	assert.False(t, HasReceiver(reflect.TypeOf(func(error, int) {}), reflect.ValueOf(new(testdata.Receiver))))
}

func TestIsBeanReceiver(t *testing.T) {
	assert.False(t, IsBeanReceiver(reflect.TypeOf("abc")))
	assert.False(t, IsBeanReceiver(reflect.TypeOf(new(string))))
	assert.True(t, IsBeanReceiver(reflect.TypeOf(errors.New("abc"))))
	assert.False(t, IsBeanReceiver(reflect.TypeOf([]string{})))
	assert.False(t, IsBeanReceiver(reflect.TypeOf([]*string{})))
	assert.True(t, IsBeanReceiver(reflect.TypeOf([]fmt.Stringer{})))
