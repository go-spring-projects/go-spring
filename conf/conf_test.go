...
func TestLoad(t *testing.T) {
	...
	_, err = Load("testdata/error.yaml")
...
func TestBytes(t *testing.T) {
	...
	_, err := Bytes([]byte("a=b"), ".xyz")
	assert.Error(t, err, "unsupported file type \\.xyz")

	_, err = Bytes([]byte("a:=b"), ".yaml")
	assert.Error(t, err, "cannot unmarshal \\!!str `a:=b` into map\\[string]interface \\{}")

	_, err = Bytes([]byte("a=b"), ".properties")
	assert.Nil(t, err)
}
...
func TestProperties(t *testing.T) {
	...
	assert.Equal(t, p.Get("float", Def("3.0")), "3.0")
}
